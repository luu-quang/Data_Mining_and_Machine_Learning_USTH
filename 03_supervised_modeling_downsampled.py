...
# ----------------------------------------------------------------------
"""
TEAMMATE STUDY GUIDE (Final Architecture):
1. Chronological Isolation: Temporal lags are strictly grouped by Customer ID (ncodpers) 
   to prevent cross-sectional data leakage between different users.
2. Split-Before-Resample Rule: We split the raw data into Train and Test FIRST. 
   The Test set is left completely untouched to reflect real-world distributions.
3. Training Acceleration: We downsample the massive training set to 1.5 million rows. 
   This cuts training time by 90% while providing maximum signal density.
4. PCA Isolation: StandardScaler and PCA are strictly `.fit()` on the TRAIN data, 
   and only `.transform()` is applied to the TEST data.
"""
...
# [2] Pre-Split Feature Engineering: Temporal Momentum Lags (Leakage-Free)
print("--- Engineering Temporal Lags ---")

# STEP 1: Sort by Customer ID and Date to ensure strict chronological order
df = df.sort_values(by=['ncodpers', 'fecha_dato'])

# STEP 2: Define the 5 target products
targets = ['ind_tjcr_fin_ult1', 'ind_cco_fin_ult1', 'ind_recibo_ult1', 'ind_nomina_ult1', 'ind_nom_pens_ult1']
lag_features = []

# STEP 3: Create isolated Lags and Momentum for EACH product per individual customer
for col in targets:
    print(f" -> Calculating chronological lags for {col}...")
    
    # Use groupby to lock the shift operation within each customer's unique timeline
    ...
    # Calculate the behavioral momentum for this specific product
    df[f'{col}_momentum'] = df[f'{col}_lag_1'] + df[f'{col}_lag_2'] + df[f'{col}_lag_3']
    
    # Append the new column names to our feature list
    lag_features.extend([f'{col}_lag_1', f'{col}_lag_2', f'{col}_lag_3', f'{col}_momentum'])

# STEP 4: Define base attributes and PCA-bound attributes
base_features = ['age', 'renta'] + lag_features
pca_source_features = ['age', 'renta'] + [c for c in df.columns if c.startswith('ind_') and c.endswith('ult1') and c not in targets]

# Define the final X and y matrices before splitting
...
# [5] Isolated PCA Feature Extraction (Zero Data Leakage)
...
# [8] Evaluate Standard Multi-Label Classification Metrics & MAE
...
# Final RAM Cleanup
del X_train_final, X_test_final, y_probs, y_pred_binary
gc.collect()